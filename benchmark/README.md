...
- nine deterministic morphed probe images selected against the pinned reference
  recognizer at normalized targets of 95, 90, 85, 80, 75, 70, 65, 60, and 55
  percent; and
...
- For image generation, a CUDA-capable GPU and substantial RAM/VRAM are
  recommended. Stable Diffusion 1.5 is substantially smaller than the previous
  generators but still requires several gigabytes of model storage and memory.
...
The pinned generator is Stable Diffusion 1.5 under the CreativeML OpenRAIL-M
license. It is pinned to an immutable Hugging Face
...
Stable Diffusion 1.5 generates at its native 512×512 resolution. The tool then
uses deterministic Lanczos resampling to create the 1024×1024 benchmark
artifact before face detection, scoring, hashing, and target selection.
...
faces. The actual selected seed is recorded in the manifest. Enrollment
detection remains strict. For intentionally degraded probes, the tool uses a
deterministic center crop when blur or occlusion prevents Haar detection; the
source transforms guarantee the enrolled face remains centered. SFace remains
the external reference recognizer and is checksum-verified after download.
Generation fails if that checksum is wrong, no unambiguous enrollment can be
...
Evaluation probes are generated by blending an enrollment with deterministic
donor identities from the evaluation split, then selecting the closest
reference-calibrated result. The morph fraction and donor identity are recorded
...
- Stable Diffusion 1.5's OpenRAIL-M license permits commercial use subject to use-based
...
| `Bundled reference detector found no face` | Confirm the tool reports version 1.3.2 or later. Probe scoring now falls back to a deterministic center crop, while enrollment generation remains strict and retries derived seeds. |
